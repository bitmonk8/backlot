...
**All four core operations and CLI implemented. Reel observability fields integrated.**
...
- Public API (`vault/src/lib.rs`):
  ...
  - `SessionMetadata`, `TranscriptTurn`, `TranscriptToolCall`, `TurnUsage` types for observability
...
- Librarian module (`vault/src/librarian.rs`):
  - `DerivedProducer` and `QueryResponder` traits (split by operation type); both return `SessionMetadata`
  - `ReelLibrarian` production implementation using reel `Agent`; captures `RunResult` and converts to `SessionMetadata`
...
- Bootstrap operation (`vault/src/bootstrap.rs`):
  ...
  - Returns `(Vec<DerivedValidationWarning>, SessionMetadata)`
...
- Record operation (`vault/src/record.rs`):
  ...
  - Returns `(Vec<DocumentRef>, Vec<DerivedValidationWarning>, SessionMetadata)`
  - Partial failure semantics (raw persists, changelog skipped on librarian failure)
- Query operation (`vault/src/query.rs`):
  ...
  - Returns `(QueryResult, SessionMetadata)`
- Reorganize operation (`vault/src/reorganize.rs`):
  ...
  - Returns `(ReorganizeReport, Vec<DerivedValidationWarning>, SessionMetadata)`
...
- CLI (`vault-cli/src/main.rs`):
  ...
  - JSON output to stdout with `usage` block (input/output tokens, cache tokens, cost, latency, tool calls)
  - `--verbose` global flag includes session transcript in `usage` output
  - Error JSON to stderr, validation warnings as plain text to stderr
  ...
  - Unit tests for config parsing, mode conversion, usage JSON building, error paths
- Shared test infrastructure (`vault/src/test_support.rs`):
  ...
  - All mocks return `SessionMetadata::empty()` for observability compatibility
  - Each mock implements only its relevant trait (no dead stubs)
- Test coverage: 121 tests (storage, prompts, librarian, bootstrap, record, query, reorganize, Vault::new, SessionMetadata, CLI)

## What Remains

Nothing. All operations, CLI, and observability are implemented.
